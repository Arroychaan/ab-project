...
export default function SambutanClient({ pageData }) {
  ...
  const content = pageData ? JSON.parse(pageData.data) : {
    ...
    paragraphs: [
      "Puji syukur kehadirat Allah SWT yang senantiasa melimpahkan taufiq dan hidayah-Nya...",
    ],
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", borderBottom: "2px solid #e2f0e9", paddingBottom: "20px" }}>
            <h1 style={{ fontSize: "28px", color: "#0d6e3f", fontWeight: "700", marginBottom: "8px" }}>
              {pageData?.title || "Sambutan Khadimul Ummah"}
            </h1>
            <p style={{ fontSize: "14px", color: "#666", fontStyle: "italic" }}>
              {pageData?.subtitle || "Lembaga Pengembangan Dakwah (LPD) Al-Bahjah"}
            </p>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "row", gap: "30px", flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            <div style={{ flex: "1", minWidth: "220px", display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
              <div style={{ 
                ...
                <img 
                  src={content.photo?.src || "/buya-yahya.jpg"} 
                  alt={content.photo?.alt || "Buya Yahya"} 
                  style={{ width: "100%", height: "100%", objectFit: "cover" }}
                />
              </div>
              <div style={{ textAlign: "center" }}>
                <h3 style={{ fontSize: "16px", color: "#0d6e3f", fontWeight: "700" }}>{content.personName}</h3>
                <p style={{ fontSize: "12.5px", color: "#777", fontWeight: "500" }}>{content.personTitle}</p>
              </div>
            </div>
            ...
            <div style={{ flex: "2", minWidth: "300px", display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              {content.paragraphs?.map((p, idx) => (
                <p key={idx} style={{ fontSize: "14.5px", color: "#444", lineHeight: "1.7" }}>
                  {p}
                </p>
              ))}
